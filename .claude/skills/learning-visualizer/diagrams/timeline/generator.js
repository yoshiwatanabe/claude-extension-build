...
function generateTimelineDiagram(jsonData, outputPath = null) {
    try {
        if (!jsonData.type || jsonData.type !== 'timeline') {
            throw new Error('Invalid diagram type. Expected "timeline"');
        }
...
if (require.main === module) {
    ...
    try {
        ...
        const result = generateTimelineDiagram(jsonData, outputFile);

        if (result.success) {
            console.log(`✓ Timeline diagram generated successfully`);
...
module.exports = generateTimelineDiagram;

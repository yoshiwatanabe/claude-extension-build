...
function generateFlowDiagram(jsonData, outputPath = null) {
    try {
        if (!jsonData.type || jsonData.type !== 'flow') {
            throw new Error('Invalid diagram type. Expected "flow"');
        }
        if (!Array.isArray(jsonData.nodes) || !Array.isArray(jsonData.edges)) {
            throw new Error('Missing required fields: nodes, edges');
        }
...
if (require.main === module) {
    ...
    try {
        ...
        const result = generateFlowDiagram(jsonData, outputFile);

        if (result.success) {
            console.log(`✓ Flow diagram generated successfully`);
...
module.exports = generateFlowDiagram;
